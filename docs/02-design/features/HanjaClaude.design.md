...
- 기본값: `{ order: "sequential", startFace: "front" }`
...
- 용도: 홈 화면 / 오답방 그리드 셀
- Props: `{ card, selected?, onTap?, onLongPress?, deleteMode?, onDelete? }`
- 표시: 한자(크게) + 뜻음(작게) + 번호(우상단)
- 선택 시: blue border
...
```
useHanjaData() → cards
useSelection(cards) → selectedIds, toggleCard, toggleAll

[상단] 타이틀 + 오답방 버튼 + 설정 버튼
[그리드] 5열 × HanjaCardCell (선택 상태)
[하단] BottomBar (선택수 / 전체선택 / 플레이)

플레이 버튼 → router.push(`/play?ids=${selectedIds.join(",")}`)
```
...
[그리드] 5열 × HanjaCardCell (deleteMode 지원)
...
```
localStorage
  hanjaPlayOptions  ←→  usePlayOptions
...
| 결정 | 이유 |
|------|------|
| goToCard vs swipeToCard 분리 | 버튼 이동은 시작면 리셋, 스와이프는 현재 면 유지 (F-13) |
| optionsRef + resolveStartFace | 카드마다 독립 랜덤 결정, 클로저 stale 방지 (F-09) |
| isWrong을 wrongIds로 계산 | 실시간 토글 상태 반영, 별도 wrongAdded 불필요 (F-15) |
| ids를 URL QueryString으로 전달 | 홈/오답방 → 플레이 진입 시 카드셋 공유 |
